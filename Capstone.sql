...
ALTER TABLE ONLY customercustomerdemo
    ADD CONSTRAINT fk_customercustomerdemo_customerdemographics FOREIGN KEY (customer_type_id) REFERENCES customerdemographics;
...
-- ANALYSIS FOR POWER BI ---

-- 1) Sales people performance analysis.

-- from "select to_char(max(order_date :: date), 'DD-MM-YYYY') from orders"
-- Conducted the analysis based on the date 06-05-1998
select 
	concat(first_name, ' ', last_name) as employee_name,
	title,
	age('07-05-1998':: date,hire_date:: date) as working_duration,
	city,
	country,
	o.order_id,
	order_date,
	round ((unit_price * quantity)* (1-discount)) as total_sales_amount,
	discount,
	round((unit_price * quantity)*discount) as discount_amount
...
select 
	o.order_id,
	age(shipped_date::date,order_date::date) as delivery_time,
	ship_city,
	ship_country,
...
select o.order_id,
		...
		ship_country,
		round ((od.unit_price * quantity)* (1-discount)) as total_sales_amount,
		round((od.unit_price * quantity)* discount) as discount_amount,
		discount,
		pr.product_name,
		pr.supplier_id,
		cat.category_id,
...
-- 5- Sales amounts by category for the highest and lowest performing sales employees.

-- Margaret Peacock (Top-performing in terms of sales.)
...
-- Steven Buchanan (Lowest-performing in terms of sales.)
...
-- 6- Average product prices by category.
...
-- 7- Price distribution of products with the highest and lowest average prices.
-- Meat/Poultry (Category with the highest average price.)
select 
		product_name,
